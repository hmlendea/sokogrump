...
namespace SokoGrump.Windows
{
    public partial class GameWindow : Gtk.Window
    {
        ...
        {
            string path = System.IO.Path.Combine(Globals.DataPath, "Levels", level + ".lvl");
        ...
        void CheckCompletion()
        {
            if (game.Completed)
            {
                if (File.Exists(System.IO.Path.Combine(Globals.DataPath, "Levels", (game.Level + 1) + ".lvl")))
                {
        ...
        void DrawLevelWon()
        {
            ...
            g.DrawImage(Image.FromFile(System.IO.Path.Combine(Globals.DataPath, "Resources", "Images", "grumpy_cat_good.jpg")), 0, 0);
            g.Dispose();
        }
